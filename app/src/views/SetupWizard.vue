...
<template>
  <v-container fluid>
    ...
    <v-stepper v-model="currentStepValue" class="mt-4 wizard-stepper" hide-actions aria-label="Setup Wizard Steps">
      ...
      <v-stepper-window class="mt-1">
        <template v-for="step in wizardSteps" :key="`window-${step.value}`">
          <v-stepper-window-item :value="step.value" class="wizard-step-content-wrapper">
            <template v-if="step.value === 'family'">
              <v-card flat class="wizard-step-card">
            ...
            <template v-else-if="step.value === 'entities'">
              <v-card flat class="wizard-step-card">
                ...
                <v-card-text>
                  ...
                  <v-list v-if="savedEntities.length > 0" class="mb-4 full-width">
                    <v-row class="full-width align-center">
                      <v-col class="text-h6">Saved Entities</v-col>
                      <v-col cols="auto" class="text-right">
                        <v-btn density="compact" variant="plain" icon="mdi-plus" color="success" @click="initNewEntity" aria-label="Add New Entity"></v-btn>
                      </v-col>
                    </v-row>
                    <v-list-item
                  ...
                  <v-btn v-else color="primary" @click="initNewEntity" class="mb-4" aria-label="Add New Entity"> Add New Entity </v-btn>
                  <entity-form
                ...
                <v-card-actions class="pa-4">
                  <v-btn color="secondary" variant="text" @click="navigateStep('back')"> Back </v-btn>
                  <v-spacer />
                  <v-btn color="primary" @click="navigateStep('next')" class="ml-2"> Next </v-btn>
                </v-card-actions>
              </v-card>
            </template>
            ...
            <template v-else-if="step.accountType">
              <v-card flat class="wizard-step-card">
                ...
                <v-card-actions class="pa-4">
                  <v-btn color="secondary" variant="text" @click="navigateStep('back')"> Back </v-btn>
                  <v-spacer />
                  <v-btn v-if="!isLastStep && step.accountType" color="secondary" variant="text" @click="navigateStep('next')" class="ml-2"> Skip </v-btn>
                  <v-btn v-if="!isLastStep" color="primary" @click="navigateStep('next')" class="ml-2"> Next </v-btn>
                  <v-btn v-else color="primary" @click="finishSetup" :loading="finishingSetup" class="ml-2"> Finish </v-btn>
                </v-card-actions>
              </v-card>
            </template>
    ...
    <v-dialog v-model="showUnsavedEntityDialog" max-width="450px" persistent></v-dialog>

    <!-- Account Dialog -->
    <v-dialog v-model="showWizardAccountDialog" max-width="600px" persistent>
      <v-card>
        <v-card-title> {{ editMode ? "Edit" : "Add" }} {{ newWizardAccount.type }} Account </v-card-title>
        <v-card-text>
          <AccountForm
    ...
    <v-snackbar v-model="snackbar.show" :color="snackbar.color" timeout="3000" location="top right"></v-snackbar>
  </v-container>
</template>
