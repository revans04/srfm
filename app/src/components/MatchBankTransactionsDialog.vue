...
<template>
  <v-dialog v-if="isReady" :model-value="props.showDialog" :fullscreen="isMobile" persistent @update:model-value="closeDialog($event)">
    <v-card>
      <v-card-title>Match Bank Transactions</v-card-title>
      <v-card-text>
        <div class="row">
          <div class="col"><v-spacer></v-spacer></div>
          <div class="col-auto text-right">
            <v-btn color="error" @click="closeDialog(false)" :disabled="props.matching" variant="plain">
              <v-icon>mdi-close</v-icon>
            </v-btn>
          </div>
        </div>
        <v-tabs v-model="activeTab" grow>
          <v-tab value="smart-matches">Smart Matches</v-tab>
          <v-tab value="remaining">Remaining Transactions</v-tab>
        </v-tabs>

        <v-window v-model="activeTab">
          <!-- Smart Matches Tab -->
          <v-window-item value="smart-matches">
            <v-row v-if="smartMatches.length > 0" class="mt-4">
              <v-col>
                ...
                <v-row class="mb-4">
                  ...
                  <v-col cols="12" md="4">
                    <v-btn :color="smartMatchesSortDirection === 'asc' ? 'primary' : 'secondary'" @click="toggleSmartMatchesSortDirection">
                ...
                <v-btn color="primary" @click="confirmSmartMatches" :disabled="selectedSmartMatchIds.length === 0 || props.matching" :loading="props.matching">
                  Confirm Selected Matches ({{ selectedSmartMatchIds.length }})
                </v-btn>
              </v-col>
            </v-row>
            <v-row v-else class="mt-4">
              <v-col>
                <v-alert type="info"> No smart matches found. Check Remaining Transactions for potential conflicts. </v-alert>
              </v-col>
            </v-row>
          </v-window-item>

          <!-- Remaining Transactions Tab -->
          <v-window-item value="remaining">
            <v-row v-if="remainingImportedTransactions.length > 0" class="mt-4">
              <v-col>
                ...
                <v-row class="mt-4">
                  <v-col cols="12" md="4">
                    <v-text-field v-model="searchAmount" label="Amount" type="number" variant="outlined" readonly></v-text-field>
                  </v-col>
                  <v-col cols="12" md="4">
                    <v-text-field v-model="searchMerchant" label="Merchant" variant="outlined" @input="searchBudgetTransactions"></v-text-field>
                  </v-col>
                  <v-col cols="12" md="4">
                    <v-text-field
                ...
                <v-row class="mt-4">
                  <v-col>
                    <v-btn color="primary" @click="toggleSplitTransaction" :disabled="props.matching">
                ...
                <v-form v-if="showSplitForm" ref="splitForm" @submit.prevent="saveSplitTransaction" class="mt-4">
                  <v-row v-for="(split, index) in transactionSplits" :key="index" class="align-center">
                    <v-col cols="12" md="3">
                      <v-select
                    ...
                    <v-col cols="12" md="1">
                      <v-btn color="error" icon="mdi-close" @click="removeSplit(index)" variant="plain"></v-btn>
                    </v-col>
                  </v-row>
                  <v-alert v-if="remainingSplitAmount !== 0" :type="remainingSplitAmount < 0 ? 'error' : 'warning'" class="mb-4">
                  ...
                  <v-btn color="primary" @click="addSplitTransaction">Add Split</v-btn>
                  <v-btn color="success" type="submit" :disabled="remainingSplitAmount !== 0 || props.matching" :loading="props.matching" class="ml-2">
                    Save Splits
                  </v-btn>
                </v-form>

                <!-- Potential Matches -->
                <v-row v-if="!showSplitForm" class="mb-4">
                  <v-col cols="12" md="4">
                    <v-select
                  ...
                  <v-col cols="12" md="4">
                    <v-btn :color="potentialMatchesSortDirection === 'asc' ? 'primary' : 'secondary'" @click="togglePotentialMatchesSortDirection">
                ...
                <div v-else-if="!showSplitForm" class="mt-4">
                  <v-alert type="info" class="mb-4"> No potential matches found. Adjust the search criteria or add a new transaction. </v-alert>
                  <v-btn color="primary" @click="addNewTransaction" :disabled="props.matching"> Add New Transaction </v-btn>
                </div>
              </v-col>
            </v-row>
            <v-row v-else class="mt-4">
              <v-col>
                <v-alert type="success"> All bank transactions have been matched or ignored. </v-alert>
              </v-col>
            </v-row>
          </v-window-item>
        </v-window>
      ...
      <v-card-actions>
        <v-btn v-if="remainingImportedTransactions.length > 0" color="warning" @click="ignoreBankTransaction" :disabled="props.matching"> Ignore </v-btn>
        <v-btn v-if="remainingImportedTransactions.length > 0" color="secondary" @click="skipBankTransaction" :disabled="props.matching"> Skip </v-btn>
        <v-spacer></v-spacer>
        <v-btn color="error" @click="closeDialog(false)" :disabled="props.matching"> Close </v-btn>
      </v-card-actions>
    </v-card>
