...
- **Bypassing the transfer model for inter-category moves.** A category-to-category transfer is a `transactionType: 'transfer'` with signed splits — negative on the source, positive on the destination. Do not record one as a paired income/expense. Aggregations branching on `isIncome` must also branch on `transactionType === 'transfer'`; see `q-srfm/src/utils/reportAggregations.ts`.
- **Modeling goal-funded expenses as transfers.** Goal funding is *not* a transfer. A goal-funded expense is a standard `Transaction` recorded on the destination category (counts toward that category's spent / available) with `funded_by_goal_id` persisted on the row. The goal's `savedToDate` / `spentToDate` are derived server-side in `GoalService.GetGoals` (the `funded_agg` sub-aggregation) and listed in `GoalService.GetGoalDetails`. New write paths must persist `fundedByGoalId`; do not auto-convert to a transfer at save time. Historical goal-funded transactions saved as transfers still work — the goal reader supports both shapes.
