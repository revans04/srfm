import type { BudgetCategory, Goal } from '../types';
 ...
 */
export function inferGoalFundingFromCategory(
  budget: { categories: BudgetCategory[] },
  txCategoryNames: string[],
  goals: Goal[],
): Goal | null {
  ...
  for (const name of names) {
    ...
    if (cat?.fundingSourceGoalId) goalIds.add(cat.fundingSourceGoalId);
    else return null;
  }

  if (goalIds.size !== 1) return null;
