/**
 * Pure derivation of Getting Started checklist completion from raw stores.
 *
 * Lifted out of `useTourStore` so the rules can be unit-tested without
 * spinning up a Pinia harness. The store wraps `computeChecklistCompletion`
 * around live store data; tests pass synthetic inputs.
 */
import type { Budget, Family } from '../types';
...
export interface ChecklistInputs {
  /** All loaded budgets across the family. Order doesn't matter. */
  budgets: Budget[];
  /** Family record, or undefined if the user has no family yet. */
  family?: Family;
  /** Subset of the Firebase user object that the checklist cares about. */
 ...
 */
export function computeChecklistCompletion(inputs: ChecklistInputs): ChecklistCompletion {
