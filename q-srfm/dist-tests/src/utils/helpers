...
export function stringToFirestoreTimestamp(dateString) {
 ...
 * @returns Formatted date string (e.g., "March 2025")
 */
export function formatDateMonthYYYY(date) {
    const d = typeof date === 'string' ? new Date(date) : date;
 ...
 */
export function toBudgetMonth(dateInput) {
...
export function getImportedTransactionDate(tx) {
    if (!tx)
        return '';
...
export function adjustTransactionDate(originalDate, newMonth, interval) {
...
export function generateDailyTransactions(transaction, newMonth) {
    const [newYear, newMonthNum] = newMonth.split('-').map(Number);
...
export function generateBiWeeklyTransactions(transaction, newMonth) {
...
export function generateWeeklyTransactions(transaction, newMonth) {
    const [newYear, newMonthNum] = newMonth.split('-').map(Number);
    const transactions = [];
    const originalDate = new Date(transaction.date);
    const originalDayOfWeek = originalDate.getDay(); // 0 (Sunday) to 6 (Saturday)
