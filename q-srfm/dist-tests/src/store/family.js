...
export const useFamilyStore = defineStore('family', () => {
    ...
    function setGroupsForEntity(entityId, groups) {
        if (!entityId)
            return;
    ...
    async function loadGroups(entityId, options = {}) {
        if (!entityId)
            return [];
    ...
    async function createGroup(entityId, payload) {
    ...
    async function renameGroup(entityId, groupId, name) {
    ...
    async function updateGroup(entityId, groupId, payload) {
        const updated = await dataAccess.updateGroup(entityId, groupId, payload);
        const list = (groupsByEntity.value[entityId] || []).map((g) => (g.id === groupId ? updated : g));
        setGroupsForEntity(entityId, list);
        return updated;
    }
    async function reorderGroups(entityId, groupIds) {
