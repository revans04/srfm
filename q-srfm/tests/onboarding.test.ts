...
import type { Budget, BudgetCategory, Family, Transaction, Account } from '../src/types.js';

// Family.members[] is structurally typed in `src/types.ts:4-12` (no exported
// UserRef). Mirror the inline shape here so the composite-scenarios test
// can build members without importing private types.
type FamilyMember = Family['members'][number];
...
describe('computeChecklistCompletion — composite scenarios', () => {
  test('a fully-onboarded user has all 7 items ticked', () => {
    const partner: FamilyMember = { uid: 'partner-uid', email: 'p@example.com', role: 'member' };
    const family = makeFamily({
      members: [
        { uid: 'owner-uid', email: 'owner@example.com', role: 'owner' },
        partner,
    ...
    for (const key of Object.keys(result)) {
      assert.equal(result[key as keyof typeof result], true, `${key} should be true`);
    }
  });
